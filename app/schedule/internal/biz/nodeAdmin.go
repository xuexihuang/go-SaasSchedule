...
func (a *AdminNode) generateSetCommand(domain string, imageTag string, tenantId string) (interface{}, error) {

	c := UserConfig{}
	c.Ingress.Enabled = true
	c.Ingress.Hosts = make([]*Hosts, 1)
	c.Ingress.Hosts[0] = &Hosts{Host: domain, Paths: []*Paths{&Paths{Path: "/adminapi(/|$)(.*)", PathType: "ImplementationSpecific"}}}
	...
	c.SqlConfig.Sql = a.generateInitSql()
	return &c, nil
}
